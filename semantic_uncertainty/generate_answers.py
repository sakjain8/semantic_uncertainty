...
def main(args):
    ...
    if args.dataset == 'svamp':
        # SVAMP requires using the context to make sense of the math word problem.
        if not args.use_context:
            logging.info('Forcing `use_context=True` for svamp dataset.')
            args.use_context = True

    elif args.dataset == 'multiwoz':
        # For MultiWOZ, we definitely want to feed the dialogue history as context.
        if not args.use_context:
            logging.info('Forcing `use_context=True` for multiwoz dataset.')
            args.use_context = True

    elif args.dataset == 'squad':
        # For SQuAD they force answerable_only=True in the original code.
    ...
    # Load main dataset via central loader (handles multiwoz, squad, triviaqa, etc.)
    train_dataset, validation_dataset = load_ds(
        args.dataset, add_options=args.use_mc_options, seed=args.random_seed
    )

    # Optional: use an OOD dataset for few-shot prompts / p_true training.
    ...
    if args.answerable_only:
        # Drop unanswerable examples from validation if required
    ...
    use_brief = args.brief_always if args.enable_brief else True

    prompt = utils.construct_fewshot_prompt_from_indices(
        train_dataset, prompt_indices, BRIEF, use_brief, make_prompt
    )
    ...
    for dataset_split in ['train', 'validation']:
        ...
        for index in tqdm(indices):
            # Small memory cleanup every 10 iterations
            if ((it + 1) % 10) == 0:  # fixed parentheses
                gc.collect()
            ...
            # Sample one low-temp answer (for accuracy) + num_generations high-temp
            ...
            # Optional: compute p_true online for validation
        ...
        # Save p_true-based uncertainty measures for validation split, if required.
    ...
    # Save info about the experiment (indices, prompts, etc.)
...
if __name__ == '__main__':
    ...
    # First: sample generations from the LLM.
    logging.info('STARTING `generate_answers`!')
    main(args)
    logging.info('FINISHED `generate_answers`!')

    # Then: compute uncertainty measures if requested.
    if args.compute_uncertainties:
        args.assign_new_wandb_id = False
        gc.collect()
        torch.cuda.empty_cache()
